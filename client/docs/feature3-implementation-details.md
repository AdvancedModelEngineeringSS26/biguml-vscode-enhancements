# Feature 3 VS Code Integration Report

Feature 3 covered several topics. The implemented work focused on:
...
Viewport persistence was part of the Feature 3 topic list, but viewport persistence is not implemented.

## Files Changed By This Work

The main files affected by this work are:
...
The `UmlDiagramCustomDocument` interface is added:
```ts
...
In `openCustomDocument`, a custom document object is created with the real URI, optional backup URI, optional restore URI, and a cleanup-aware `dispose` function.
...
In `resolveCustomEditor`, the GLSP model URI was changed from always using `document.uri` to using a temporary restore model URI:
...
In `saveCustomDocument`, the destination for UML documents that have a restore file is changed:
...
In `revertCustomDocument`, restore-file reset behavior is added before asking the connector to reload:
...
The following cleanup helpers are added:
...
`prepareGLSPClient` is changed to accept `modelUri` and serialize that URI into the `GLSPDiagramIdentifier`:
...
During the architecture merge, the revert logic is moved into the contribution `DocumentManager`. The final code dispatches a `RequestModelAction` with:
...
The `revertSourceUri` helper is introduced:
...
The model update listener handling is also changed:
...
Diagram keybindings are changed from only checking:
...
Then it is listened for focus-state actions from the GLSP client:
...
and registered as an extension action kind:
...
During testing, some diagram shortcuts work and some still do not behave correctly.
...
Viewport persistence was listed in Feature 3, but it is not yet implemented.
...
This bug is related to the `N` shortcut for the primary element navigator.
